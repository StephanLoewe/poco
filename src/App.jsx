  ...
  return (
    <div onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 11, padding: "8px 0 8px 34px", borderBottom: `1px solid ${T.border}`, cursor: "pointer", opacity: done ? 0.5 : 1, transition: "opacity 0.15s" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13.5, color: done ? T.dim : T.text, textDecoration: done ? "line-through" : "none", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>{task.title}</div>
...
function ListView({ tasks, date, onTaskClick, onAdd, onToggleDone }) {
  ...
  const overdue = tasks.filter(t => t.date && t.date < today && t.status === "open" && !t.parentId && !t.allDay)
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "14px 16px 32px" }}>
      ...
      {days.map(d => {
        const dk   = dKey(d)
        const isT  = dk === today
        const dt   = tasks.filter(t => t.date === dk && !t.parentId && !t.allDay).sort((a, b) => a.time.localeCompare(b.time))
    ...
    <div onClick={onClick} style={{
      ...
      <div
        ...
        style={{
          ...
          border: `1.5px solid ${done ? lc.pastelText : T.muted}`,
...
export default function App() {
  ...
  return (
    <>
      ...
      `}</style>
      <div style={{ fontFamily: "system-ui, sans-serif", background: T.bg, color: T.text, height: "100%", display: "flex", flexDirection: "column", overflow: "hidden", paddingBottom: "env(safe-area-inset-bottom)" }}>
