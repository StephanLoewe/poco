...
// ─── Header ───────────────────────────────────────────────────
function Header({ view, setView, date, setDate, onAdd, syncing, onSync, onLogin, authed }) {
    ...
    <div style={{ padding: "max(14px, env(safe-area-inset-top)) 16px 10px", background: T.surface, borderBottom: `1px solid ${T.border}`, flexShrink: 0, boxShadow: "0 1px 0 rgba(0,0,0,0.04)" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {!authed && (
            <button onClick={onLogin} title="Mit Google anmelden"
              style={{ height: 34, padding: "0 10px", borderRadius: 10, border: "1px solid rgba(37,99,235,0.3)", background: "rgba(37,99,235,0.06)", color: "#2563EB", cursor: "pointer", fontSize: 11, fontWeight: 600, fontFamily: "inherit", display: "flex", alignItems: "center", gap: 5 }}>
    ...
    <>
      ...
      <div style={{ fontFamily: "'Plus Jakarta Sans', system-ui, sans-serif", background: T.bg, color: T.text, height: "100dvh", maxWidth: 430, margin: "0 auto", display: "flex", flexDirection: "column", overflow: "hidden", paddingBottom: "env(safe-area-inset-bottom)" }}>
        <Header view={view} setView={setView} date={date} setDate={setDate}
          onAdd={() => setModal({ task: view === "inbox" ? { date: "" } : {} })} syncing={syncing} onSync={doSync} onLogin={doSync} authed={authed} />
